    ...
    public class CraneJoystick : MonoBehaviour
    {
        ...
        private void ApplyJoystickValues()
        {
                ...
                if (CraneBridgeLocText != null)
                    {
                    CraneBridgeLocText.text = (Math.Round(Bridge.transform.localPosition.x)).ToString();
                    CraneTrolleyLocText.text = (Math.Round(Trolley.transform.localPosition.z)).ToString();
                }
